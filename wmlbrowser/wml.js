...
//	if(typeof escaping=='undefined'){return str;}	//修改:否则下一行出错
    ...
    str = str.replace
        (matcher,
         function (lastMatch, s1, s2, s3, s4, s5, s6) {
			...
			//修改:对于不匹配的组 Firefox 返回空字符串但其他浏览器返回 undefined
             ...
             var valName     = (typeof s3 == 'undefined') ? s6 : s3;
            ...
            var valValue = (typeof valName != 'undefined') ? nsWMLgetValue(valName,mode,valFormName) : "";
...
function nsWMLKeyDown4Select(e){
	var ks='37,38,39,40',
		k=e.keyCode;
	if(ks.indexOf(k)<0) return false;
	k=Number(k);
	if(k==38){
		e.target.focus();
		return false;
